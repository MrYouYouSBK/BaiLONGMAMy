...
function getApiHost() {
  const envHost = String(globalThis.process?.env?.GAI_HOST || globalThis.process?.env?.BAILONGMA_HOST || '').trim()
...
function isLanAccessEnabled() {
  return getNetworkConfig().allowLanAccess
    || /^(1|true|yes|on)$/i.test(String(globalThis.process?.env?.GAI_ALLOW_LAN || globalThis.process?.env?.BAILONGMA_ALLOW_LAN || '').trim())
}
...
function getAuthToken() {
  return String(globalThis.process?.env?.GAI_API_TOKEN || globalThis.process?.env?.BAILONGMA_API_TOKEN || '').trim()
}
